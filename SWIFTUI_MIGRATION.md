...
旧Tkinter UIは、移行済み機能では再利用せず、未移行機能のフォールバックとして `./run_legacy_mac.sh` から起動できます。サマリー出力、検出マスクの適用と動画フレーム上でのマスク描画、検出モデルの選択、既存WCS／固定カメラ補正の適用と高精度補正データの新規作成、入力種別の優先順位、定期スキャン、RTSP運用、RTSP固定パターン補正の検証・作成・適用、NoiseTwin／時間平均の設定、動画連結はSwiftUI版で扱えます。動画連結では、入力順の変更、セーフモード、ビットレート、コーデック、FPS、実時刻表示、固定パターン補正、進捗表示、キャンセルを利用できます。AIアシスタント、詳細パラメータなどは、既存動作を壊さず順次SwiftUIへ移行します。
