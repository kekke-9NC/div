...
旧Tkinter UIは、移行済み機能では再利用せず、未移行機能のフォールバックとして `./run_legacy_mac.sh` から起動できます。サマリー出力、検出マスクの適用と動画フレーム上でのマスク描画、検出モデルの選択、既存WCS／固定カメラ補正の適用と高精度補正データの新規作成、入力種別の優先順位、定期スキャン、RTSP運用、NoiseTwin／時間平均の設定はSwiftUI版で扱えます。RTSP固定パターン補正、解析ツール、AIアシスタントなどは、既存動作を壊さず順次SwiftUIへ移行します。
